...
def load_frames(root: Path) -> dict[str, pd.DataFrame]:
    ...
    for path in sorted(root.glob("*.csv")):
        ...
        code = frame["code"].dropna().astype(str).iloc[-1] if "code" in frame.columns and frame["code"].notna().any() else path.stem
        frame["code"] = code
...
def main() -> None:
    ...
    try:
        for split in a.splits:
            ...
            date_limit = a.max_train_dates if split == "train" else a.max_val_dates if split == "val" else None
            if date_limit is not None:
                keep_dates = sorted(meta["as_of_date"].astype(str).unique())[-date_limit:]
