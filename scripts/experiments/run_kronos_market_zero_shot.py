#!/usr/bin/env python
"""Kronos zero-shot cross-sectional ranking on a parameterized current-constituent market panel.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--data-vintage', default='current constituents from configured local dataset')
...
def config_from_args(a: argparse.Namespace) -> RunConfig:
    return RunConfig(
        ...
        quantiles=a.quantiles, top_n=a.top_n, data_vintage=a.data_vintage,
    ...
    for fp in files:
        try:
            ...
            if 'code' not in df.columns:
                if 'ticker' in df.columns and fp.parent.name.lower() == 'yahoo_sp500':
                    df['code'] = 'us.' + df['ticker'].astype(str).str.strip()
                else:
                    df['code'] = df['ticker'] if 'ticker' in df.columns else fp.stem
            elif fp.parent.name.lower() == 'yahoo_sp500':
                raw_codes = df['code'].astype(str).str.strip()
                df['code'] = raw_codes.map(lambda v: v if v.lower().startswith('us.') else ('us.' + v.split('_', 1)[1] if v.lower().startswith('us_') else 'us.' + v))
            df = df[REQUIRED + ["code"]].copy()
