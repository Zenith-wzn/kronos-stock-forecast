"""Independent CSI800 A/B/C continual-learning experiment runner.
...
def _save_checkpoint(head: Any, path: Path, version: int, c: Config, validation_rankic: float) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    metadata = {
        "experiment_type": "continual_learning_ranking_head",
        "version": version,
        "validation_metric": "mean_daily_spearman_rankic",
...
def _run(c: Config) -> dict[str, Any]:
    ...
    if (c.test_start, c.test_end, c.lookback, c.pred_len) != (TEST_START, TEST_END, LOOKBACK, PRED_LEN):
        raise ValueError("locked CSI800 Kronos-paper shift requires test=2025-07-01..2026-06-05 and 90/10 windows")
    ...
    if not c.smoke_test_schedule and c.max_test_dates is None:
        if len(test_dates) != 226 or test_dates[0] != c.start or test_dates[-1] != c.end:
            raise ValueError(
                f"formal CSI800 test calendar must be exactly 226 dates, got {len(test_dates)} "
                f"from {test_dates[0].date()} to {test_dates[-1].date()}"
            )
    ...
    if not np.isfinite(b_val):
        raise ValueError("initial validation RankIC is not finite")
    c_head = b_head.clone() if hasattr(b_head, "clone") else copy.deepcopy(b_head)
    version = 0
    champion_val = float(b_val)
    checkpoint_dir = out / "checkpoints"
    _save_checkpoint(b_head, checkpoint_dir / "B_initial.pt", 0, c, b_val)
    _save_checkpoint(c_head, checkpoint_dir / "C_version_000.pt", 0, c, b_val)
    rows: list[dict[str, Any]] = []
    ...
    for pos, date in enumerate(test_dates):
        ...
        if date in update_set:
            ...
            if new_indices:
                ...
                accepted = bool(
                    np.isfinite(candidate_val)
                    and candidate_val >= champion_val - c.validation_tolerance
                )
                update_decision = "accepted" if accepted else "rejected_validation_rankic_gate"
                if accepted:
                    c_head = candidate
                    version += 1
