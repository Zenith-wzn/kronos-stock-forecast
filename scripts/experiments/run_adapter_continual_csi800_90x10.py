...
def verify_boundaries(train_m, val_m, test_m):
    ...
    if test_m.as_of_date.min() != TEST_START or test_m.as_of_date.max() != TEST_END: raise AssertionError('test interval is not locked')
...
def parse_args():
    p=argparse.ArgumentParser(); p.add_argument('--cache-dir',required=True); p.add_argument('--panel-dir',required=True); p.add_argument('--initial-checkpoint',required=True); p.add_argument('--output-dir',required=True); p.add_argument('--device',default='cuda:0'); p.add_argument('--seed',type=int,default=100); p.add_argument('--batch-size',type=int,default=32); p.add_argument('--update-epochs',type=int,default=2); p.add_argument('--head-lr',type=float,default=3e-4); p.add_argument('--adapter-lr',type=float,default=1e-4); p.add_argument('--weight-decay',type=float,default=1e-4); p.add_argument('--lambda-rank',type=float,default=.2); p.add_argument('--patience',type=int,default=2); p.add_argument('--replay-max-rows',type=int,default=120000); p.add_argument('--replay-recent-months',type=int,default=24); p.add_argument('--replay-recent-fraction',type=float,default=.70); return vars(p.parse_args())
...
def main():
    ...
    try:
        ...
        verify_locked_keys(train_m,panel,'train',True); verify_locked_keys(val_m,panel,'val',True); verify_locked_keys(test_m,panel,'test',True); verify_boundaries(train_m,val_m,test_m)
        ...
        if test_m.as_of_date.nunique() != 226: raise AssertionError(f'formal CSI800 test lock mismatch rows={len(test_m)} dates={test_m.as_of_date.nunique()}')
        ...
        expected_updates=[pd.Timestamp(x) for x in ['2025-07-31','2025-08-29','2025-09-30','2025-10-31','2025-11-28','2025-12-31','2026-01-30','2026-02-27','2026-03-31','2026-04-30','2026-05-29']]
